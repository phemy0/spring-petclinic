    ...
    stage('Checkout') {
      steps {
        ...
        stage('Clean Local Cache & Build') {
            steps {
                // Optional: Remove problematic cached dependencies
                sh '''
                    echo "Removing cached Jakarta Cache and Ehcache artifacts"
                    rm -rf ~/.m2/repository/jakarta/cache
                    rm -rf ~/.m2/repository/org/ehcache
                '''
                ...
                // Clean and build the project, forcing updates
